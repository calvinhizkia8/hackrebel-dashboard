...
export default async function handler(req, res) {
  ...
  const allowed = [process.env.DASHBOARD_URL, 'http://localhost', 'http://127.0.0.1', 'null'];
  ...
  try {
    ...
    CLIENTS.forEach((c, i) => {
      if (dataValues[i]) data[c.username] = dataValues[i];
      statuses[c.username] = {
    ...
    return res.status(200).json({ clients: CLIENTS, data, statuses, lastSync: Date.now() });
  } catch (e) {
