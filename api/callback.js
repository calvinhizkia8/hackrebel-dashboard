...
export default async function handler(req, res) {
  ...
  return res.status(200).send(htmlPage('✅ Berhasil!', `
    ...
    <br/>
    <a href="${process.env.DASHBOARD_URL}" style="display:inline-block;background:#FE2C55;color:#fff;padding:10px 22px;border-radius:8px;text-decoration:none;font-weight:700">Lihat Dashboard →</a>
  `));
}
...
async function fetchAndStoreData(account, token) {
  ...
  for (let page = 0; page < 5; page++) {
    ...
    allVideos.push(...rawVideos);
    if (!videoData?.data?.has_more) break;
  ...
  await kv.set(`data:${account}`, { profile, videos, ts: Date.now(), source: 'api' });
...
function htmlPage(title, body) {
  return `<!DOCTYPE html><html lang="id"><head>
    ...
    <style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:-apple-system,sans-serif;background:#000;color:#fff;display:flex;align-items:center;justify-content:center;min-height:100vh;padding:24px}.card{background:#111;border:1px solid #222;border-radius:16px;padding:32px;max-width:420px;width:100%;text-align:center}h1{font-size:22px;font-weight:800;margin-bottom:12px}p{font-size:14px;color:#999;line-height:1.6;margin-bottom:6px}a{color:#25F4EE}</style>
